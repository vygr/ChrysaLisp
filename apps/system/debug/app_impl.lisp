...
(defun step (%0)
	(when (elem-get %0 +debug_rec_reply_id)
		(mail-send (elem-get %0 +debug_rec_reply_id)
			(str (elem-get %0 +debug_rec_state)))))

(defun play (%0)
	(defq state (elem-get %0 +debug_rec_state))
	(elem-set %0 +debug_rec_state :play)
	(if (eql state :paused) (step %0)))

(defun forward (%0)
	(defq state (elem-get %0 +debug_rec_state))
	(elem-set %0 +debug_rec_state :forward)
	(if (eql state :paused) (step %0)))

(defun pause (%0)
	(elem-set %0 +debug_rec_state :paused))

(defun reset (&optional %0)
	(setd %0 -1)
	(if (<= 0 %0 (dec (length buf_list)))
		(progn
			(def *hslider* :value %0)
			(setq selected_index %0)
			...
			((= idx +select_service)
				...
				(cond
					((> type 0)
						...
						(bind '(& brk_id brk_cond file_name line_num &ignore) (split data "|"))

...
(defun rebuild-board ()
	...
	(each (lambda (%0)
		(defq value :nil)
		(cond
			((eql (elem-get game_map %0) "f")
				(. (defq mc (Button)) :connect (+ +event_click %0))
			...
			((eql (elem-get game_map %0) "b")
				(. (defq mc (Button)) :connect (+ +event_click %0))
				(def mc :text "" :border 1 :flow_flags
					(logior +flow_flag_align_hcenter +flow_flag_align_vcenter) :min_width 32 :min_height 32)
				(. game_grid :add_child mc))
			((eql (elem-get game_map %0) "r")
				(if (< 0 (elem-get game_board %0) 9)
					(. (defq mc (Label)) :connect (+ +event_click %0))
					(defq mc (Label)))
				(def mc :text
					(cond
						((= (defq value (elem-get game_board %0)) 0) "")
						((< 0 value 9) (str value))
						((= value 9) "X"))
...
(defun board-layout ((gw gh nm))
	...
	(defq gwh (* gw gh))
		; (ui-grid game_grid (:grid_width 1 :grid_height 5)
	(each (lambda (%0)
		(. (defq mc (Button)) :connect (+ %0 +event_click))
		(def mc :text "" :border 1 :flow_flags
			(logior +flow_flag_align_hcenter +flow_flag_align_vcenter) :min_width 32 :min_height 32)

	...
	(while (defq e (pop ranges) s (pop ranges))
		...
		(each (lambda (c)
			(def (defq c (+ s (* c grid_width)) l (create-label)) 'flow_flags (const (logior flow_flag_right flow_flag_align_vcenter))
				'border 0 'font (const (create-font "fonts/Hack-Regular.ctf" 12)) 'text (num-to-hex-str c))
...
(ui-window window ()
	(ui-flow _ ('flow_flags flow_down_fill)
		...
		(ui-flow _ ('flow_flags flow_right_fill 'color *env_toolbar_col* 'font (create-font "fonts/Entypo.ctf" 32))

...
(defun-bind main ()
	...
	(while (cond
		...
		((<= event_win_prev id event_win_next)
			(win-refresh (% (+ index (dec (* 2 (- id event_win_prev))) (length pcbs)) (length pcbs))))
		((<= event_win_scale_down id event_win_scale_up)
			(setq zoom (max (min (+ zoom (dec (* 2 (- id event_win_scale_down)))) max_zoom) min_zoom))
			(win-refresh index))

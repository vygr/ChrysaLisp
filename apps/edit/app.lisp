...
(defun-bind select-action-on-enter (f)
	...
	(cond
		((and (not (eql fpath (elem +text_fpath+ current_text))) (file-stream fpath))
			(when (eql "Yes" (confirm "Overwrite existing file?" "Yes, No" argb_red))
				(save-buffer fpath)))
		((not (file-stream fpath))
			(notify (cat "File saved to new path: " fpath ".") argb_yellow)
				(save-buffer fpath))
		(t (save-buffer fpath))))

(defun-bind save-buffer (fpath)
	(unless (or (eql fpath "") (ends-with "/" fpath) (find ":" fpath))

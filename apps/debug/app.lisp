	...
	(if (le 0 _ (dec (length vdu_list)))
		...
		(progn
			...
			(view-dirty (vdu-print vdu (cat
				...
				"Green buttons act on a single task." (ascii-char 10)
				"Cyan buttons act on all tasks." (ascii-char 10)
				"Red slider to switch between tasks." (ascii-char 10) (ascii-char 10)
	...
	(cond
		...
		((eq (defq id (get-long (defq msg (mail-mymail)) ev_msg_target_id)) event_win_debug)
			(defq reply_id (get-long msg debug_msg_reply_id)
				...
				key (sym (str (bit-shr reply_id 32) ":" tcb))

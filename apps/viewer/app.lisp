...
(ui-window mywindow (:color +argb_grey2+)
	...
	(ui-flow _ (:flow_flags +flow_right_fill+ :font *env_terminal_font*)
		(ui-scroll tree_scroll +scroll_flag_vertical+ nil
			(ui-flow tree (:flow_flags (logior +flow_flag_down+ +flow_flag_fillw+))))
...
(defun populate-tree ()
	...
	(each (#
		(def (defq entry (Button)) :text %0 :border 0 :color +argb_white+)
		(component-connect entry +event_file_button+)
		(. tree :add_child entry)) all_src_files)
...
(defun main ()
	...
	(while (cond
		...
		((= id +event_min+)
			;min button
			(window-resize vdu_min_width vdu_min_height))
		((= id +event_max+)
			;max button
			(window-resize vdu_max_width vdu_max_height))
		((= id +event_scroll+)
			;user scroll bar
		...
		((= id +event_file_button+)
			...
			(set (. current_button :dirty) :color +argb_grey12+)
			(populate-vdu (get :text current_button)))

...
(defun sphere (p c r)
	(sub (vec-flength-3d (vec-sub-3d p c)) r))
...
(defun ray-march (ray_origin ray_dir l max_l)
	...
	(while (and (lt (setq i (inc i)) 1000)
				(gt d min_distance)
				(lt l max_l))
		(defq d (scene (vec-add-3d ray_origin (vec-fscale-3d ray_dir l)))
...
(defun lighting (surface_pos surface_norm cam_pos)
	(defq obj_color (vec-floor-3d (vec-mod-3d surface_pos 2.0))
		...
		light_dis (vec-flength-3d light_vec)
		light_norm (vec-fscale-3d light_vec (fdiv 1.0 light_dis))
		light_atten (min (fdiv 1.0 (fmul light_dis light_dis 0.01)) 1.0)
		ref (vec-reflect-3d (vec-fscale-3d light_norm -1.0) surface_norm)
		ambient 0.05
		diffuse (max 0.0 (vec-fdot-3d surface_norm light_norm))
		specular (max 0.0 (vec-fdot-3d ref (vec-fnorm-3d (vec-sub-3d cam_pos surface_pos))))
		specular (fmul specular specular specular specular 0.8)
		obj_color (vec-fscale-3d obj_color (add (fmul diffuse 0.8) ambient))
		obj_color (vec-add-3d obj_color (list specular specular specular))
		light_col (vec-fscale-3d '(1.0 1.0 1.0) light_atten))
	(vec-fmul-3d obj_color light_col))
...
(defun scene-ray (ray_origin ray_dir)
	...
	(if (ge l clipfar)
		(list 0.0 0.0 0.0)
		(progn
			(defq surface_pos (vec-add-3d ray_origin (vec-fscale-3d ray_dir l))
	...
	(while (lt y h)
		...
		(while (lt x w)
			(defq
				...
				ray_dir (vec-fnorm-3d (vec-sub-3d
				(list

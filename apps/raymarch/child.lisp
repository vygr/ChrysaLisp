...
(defun ray-march (ray_origin ray_dir l max_l)
	...
	(while (and (lt (setq i (inc i)) 1000)
				(gt d min_distance)
				(lt l max_l))
		(defq d (scene (vec-add ray_origin (vec-scale ray_dir l)))
...
(defun shadow (ray_origin ray_dir l max_l k)
	...
	(while (gt (setq i (dec i)) 0)
		(defq h (scene (vec-add ray_origin (vec-scale ray_dir l)))
...
(defun lighting (surface_pos surface_norm cam_pos)
	(defq obj_color (vec-floor (vec-mod surface_pos 2.0))
		light_vec (vec-sub light_pos surface_pos)
		light_dis (vec-length light_vec)
		light_norm (vec-scale light_vec (fdiv 1.0 light_dis))
		...
		obj_color (vec-scale obj_color (add (fmul diffuse (sub 1.0 ambient)) ambient))
		obj_color (vec-add obj_color (points specular specular specular)))

...
(defun chat-scroll-page-w ()
	...
	(bind '(vsw &ignore) (if (defq vs (get :vslider *chat_scroll*))
			(. vs :get_constraint) '(16 0)))
	(max 360 (- sw vsw 16)))

(defun channel-scroll-w ()
	(bind '(sw &ignore) (. *channel_scroll* :get_size))
	(bind '(vsw &ignore) (if (defq vs (get :vslider *channel_scroll*))
			(. vs :get_constraint) '(16 0)))
	(max 180 (- sw vsw)))
...
(defun relayout-channel ()
	(defq cw (channel-scroll-w))
	(bind '(& h) (. *channel_flow* :pref_size))
	(. *channel_flow* :change_dirty 0 0 cw h :t)
...
(defun render-channel-list ()
	...
	(defq cw (channel-scroll-w)
		my_name (get :clear_text *chat_name*))
	(each (lambda (entry)
		...
		(defq del_btn (Button))
		(def del_btn :text "X" :font *env_tiny_font* :ink_color +argb_black)
			...
			(+select_main
				(cond
					...
					((= (getf *msg* +ev_msg_target_id) +event_close)
						(setq running :nil))
					((= (getf *msg* +ev_msg_target_id) +event_min)
						(bind '(x y w h)
							(apply view-fit (cat (. *window* :get_pos) (. *window* :pref_size))))
						(. *window* :change_dirty x y w h)
						(refresh-layout))
					((= (getf *msg* +ev_msg_target_id) +event_max)

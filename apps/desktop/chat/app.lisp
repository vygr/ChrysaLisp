...
(defq *config_file* (cat *env_home* "chat.tre") *config_version* 1
...
(ui-window *window* (:color +argb_grey15)
	...
	(ui-flow user_bar (:flow_flags +flow_right_fill :border 1 :color +argb_white :ink_color +argb_black :font *env_button_font*)
		(ui-label _ (:text "User:" :font *env_bold_font*))
		(. (ui-textfield chat_user (:clear_text "" :hint_text "username"))
			:connect +event_connect))
	(ui-flow _ (:flow_flags +flow_up_fill)
		(ui-flow input_bar (:flow_flags +flow_right_fill :border 1 :color +argb_white :ink_color +argb_black :font *env_button_font*)
			(ui-label _ (:text "Chat:" :font *env_bold_font*))
			(. (ui-textfield chat_text (:clear_text "" :hint_text "Type markdown message..."))
				:connect +event_send))
...
(defun render-history ()
	...
	(if (empty? *history*)
		(progn
			(defq welcome (create-chat-md "System"
				(cat "Welcome to ChrysaLisp Chat!\n"
					"Type Markdown messages below (e.g. `**bold**`, `*italic*`, `` `code` ``).\n"
					"Click the **Join** button or press Enter on your username to connect.")
				page_w))
...
(defun connect-chat ()
	...
	(unless entry
		...
		(setq entry (mail-declare (elem-get select +select_chat)
				"*Chat" "Chat Service 0.1"))
	...
	(while running
		...
		(cond
			...
			((= id +event_max)
				...
				(defq target_w (min 1024 (- mw 40))
					target_h (min 768 (- mh 40)))

...
(defun-bind build-modal ()
	...
	(each (lambda (x)
		(def (defq f (create-flow)) :flow_flags flow_right_fill)
		(def (defq l1 (create-label)) :min_width 8 :border 0)
		(def (defq b (create-button)) :flow_flags (logior flow_flag_align_hcenter flow_flag_align_vcenter)
			:border 1 :text x :min_width (+ bw 24) :min_height (+ bh 8))
		(def (defq l2 (create-label)) :min_width 8 :border 0)
		(view-add-child f l1)

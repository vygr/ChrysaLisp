...
(defun populate ()
	...
	(unless (and (= (length new_services) (length services))
		...
		(each (#
			(defq info (split %0 ","))
			(def (defq _ (create-label)) :border 1 :text (elem 0 info))
			(view-add-child service_flow _) (push service_labels _)
			(def (defq _ (create-label)) :border 1 :text (elem 1 info))
			(view-add-child mbox_flow _) (push mbox_labels _)
			(def (defq _ (create-label)) :border 1 :text (if (> (length info) 2) (elem 2 info) "No Info"))

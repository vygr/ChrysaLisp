...
(defun-bind vertex-cloud (num)
	...
	(while (> (setq num (dec num)) -1)
		(push out (list
			(i2r (- (random (const (* box_size 2))) box_size))
			(i2r (- (random (const (* box_size 2))) box_size))
			(i2r (- (random (const (* box_size 2))) box_size))
			(i2r 50)
...
(defun-bind vertex-vel (num)
	...
	(while (> (setq num (dec num)) -1)
		(push out (list
			(i2r (- (random (const (inc (* max_vel 2)))) (const max_vel)))
			(i2r (- (random (const (inc (* max_vel 2)))) (const max_vel)))
			(i2r (- (random (const (inc (* max_vel 2)))) (const max_vel)))))) out)
...
(defun-bind vertex-update (verts vels)
	(each (lambda (v s)
		...
		(if (or (> x (const (i2r box_size))) (< x (const (i2r (neg box_size)))))
			(tuple-set vertex_vel_vx s (* xv (const (i2r -1)))))
		(if (or (> y (const (i2r box_size))) (< y (const (i2r (neg box_size)))))
			(tuple-set vertex_vel_vy s (* yv (const (i2r -1)))))
		(if (or (> z (const (i2r box_size))) (< z (const (i2r (neg box_size)))))
			(tuple-set vertex_vel_vz s (* zv (const (i2r -1)))))

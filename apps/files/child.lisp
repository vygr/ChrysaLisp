...
(defun main ()
	...
	(bind '(x y w h) (apply view-locate (view-get-size mywindow)))
	...
	(while (cond
		...
		((= id +event_file_button+)
			(defq old_filename (get :text filename) new_filename (cat current_dir
				(get :text (view-find-id mywindow (get-long msg ev_msg_action_source_id)))))
		...
		((= id +event_tree_button+)
			(setq current_dir (get :text (view-find-id mywindow (get-long msg ev_msg_action_source_id))))
			(populate-files all_files current_dir exts))

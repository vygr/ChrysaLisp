...
(defun-bind normal-line ()
	(cond
		((starts-with "```" line_str)
			(setq state :code))
		((starts-with "#### " line_str)
			(def (setq line_widget (create-label)) :border 0 :text (slice 5 -1 line_str)
		...
		((starts-with "* " line_str)
			...
			(def (defq space1_widget (create-label)) :border 0 :min_width space_width)
			(def (defq bullet_widget (create-label)) :border 0 :text (num-to-utf8 0xe979) 
					:font (create-font "fonts/Entypo.ctf" 18))
			(def (defq space2_widget (create-label)) :border 0 :min_width space_width)
			(def (defq text_widget (create-label)) :border 0 :text (slice 2 -1 line_str))
		...
		((find-rev "`" line_str)
			...
			(each (lambda (word)
				...
				(cond
					((even? word_cnt)
						(def (defq word_widget (create-label)) :border 0 :text (cat " " word " ") :ink_color argb_blue))
					(t	(def (defq word_widget (create-label)) :border 0 :text word :ink_color argb_black)))

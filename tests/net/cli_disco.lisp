...
(defun run-test ()
	(print "=== ChrysaLisp Network Auto-Discovery Remote Work Test ===")
	...
	; 2. Enable auto-discovery (NO target IP/port specified!)
	(print "\nCLIENT: Enabling auto-discovery via net-discover-rpc...")
	(defq d_status (net-discover-rpc))
	(print "CLIENT: Auto-discovery enabled (status: " d_status ")")

	; 3. Wait for remote nodes to appear via auto-discovery
	...
	(when (< (length remote_nodes) expected_remote)
		(print "\n=== DISCO TEST RESULT: FAILED - insufficient remote nodes discovered ===")
		(throw "insufficient remote nodes discovered" :nil))
	...
	(print "\n=== Auto-Discovery Remote Work Test Summary ===")
	...
	(if pass
		(print "\n=== DISCO TEST RESULT: SUCCESS ===")
		(progn
			(print "\n=== DISCO TEST RESULT: FAILED ===")
			(throw "auto-discovery test verification failed" :nil))))

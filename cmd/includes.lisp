		...
		(lines! (lambda (line)
				...
				(case (first line_split)
					...
					(("call" "entry" "exit" "def-method" "gen-vtable" "gen-create")
						(merge requires (list (find-file (second line_split)))))
					(("gen-type")
						(merge requires (list "class/list/class.inc" "class/sym/class.inc")))
					(("signature")
						(merge requires (map (const find-file) (rest line_split))))
					)

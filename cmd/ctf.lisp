...
(defun print-font (font_db verbosity)
	(when font_db
		...
		(defq kern_map (. font_db :find :kern_map)
			kern_list (if kern_map (. kern_map :tolist) (list)))
		(print "\tTotal Kerning Pairs: " (length kern_list))
		(when (and (> verbosity 0) (nempty? kern_list))
			(print "\tKerning Pairs:")
			(each (lambda ((key val))
				(defq left (>>> key 32)
					right (logand key 0xffffffff)
					left_str (if (<= 32 left 126) (char left) (str "0x" (long-to-hex-str left)))
					right_str (if (<= 32 right 126) (char right) (str "0x" (long-to-hex-str right))))
				(print "\t\t" left_str " + " right_str " : " (format-fixed-24 val)))
				(sort kern_list (# (- (first %0) (first %1))))))
		(print)))

...
(defun smooth-slice ((y xmin xmax))
	(if xmax
	...
	(list y
		(if (< cur_min 500000.0) (min (or xmin cur_min) cur_min) xmin)
		(if (> cur_max -500000.0) (max (or xmax cur_max) cur_max) xmax)))
...
(defun generate-optical-kerning (font_db)
					...
					(for 33 127
						...
						(when (defq profile_b (. profiles :find c2))
							(when (defq raw_overlap (calculate-pair-kern profile_a profile_b width_a 0.0 target_gap))
								(defq dist (+ width_a raw_overlap))
								(. pair_dists :insert (+ (<< c1 8) c2) dist)
	...
	(for 33 127
		(defq c1 (!))
		(when (. profiles :find c1)
			(for 33 127
				(defq c2 (!))
				(when (defq d12 (. pair_dists :find (+ (<< c1 8) c2)))
					(for 33 127
						(defq c3 (!))
						(when (and (. profiles :find c3)
								(defq d23 (. pair_dists :find (+ (<< c2 8) c3)))
								(defq d13_min (. pair_dists :find (+ (<< c1 8) c3))))
							(defq actual_d13 (+ d12 d23))
							(when (< actual_d13 d13_min)
								(defq delta (/ (- d13_min actual_d13) 2.0))
								(setq d12 (+ d12 delta))
	...
	(each (lambda (page_db)
		(each (lambda (glyph_db)
				...
				(defq c1 (. glyph_db :find :char_code))
				(when (<= 33 c1 126)
					(defq width_a (n2f (. glyph_db :find :advance))
						text_list (list))
					(for 33 127
						(defq c2 (!))
						(when (defq dist (. pair_dists :find (+ (<< c1 8) c2)))
							(defq raw_overlap (- dist (+ width_a xkern)))
...
(defun parse-ttf-glyph (buf glyf_offset len scale_factor)
	...
	(when (> number_of_contours 0)
		...
		(while (< fi num_points)
			...
			(when (/= (logand f 8) 0)
				...
				(times repeat_count
					(set-byte flags fi f)
					(++ fi))))
		(defq coords_x (reals) curr_x +real_0 fi 0)
		(times num_points
			...
			(++ fi))
		(defq coords_y (reals) curr_y +real_0 fi 0)
		(times num_points
			(defq f (get-ubyte flags fi))
			(cond
				((/= (logand f 4) 0)
						...
						(progn
							...
							(times c_num_points
								(defq p1 (elem-get c_points pi)
									mx (* (+ (first p0) (first p1)) +real_1/2)
									my (* (+ (second p0) (second p1)) +real_1/2))
					...
					(cond
						...
						(:t
							...
							(if (third p2)
								(++ ri 2))
							(unless (third p2)
								(setq p2 (list (* (+ (first p1) (first p2)) +real_1/2) (* (+ (second p1) (second p2)) +real_1/2) :t))
								(++ ri))
		...
		(setq min_x +real_0 max_x (* (- max_x temp_min_x) scale_factor)
			min_y (* max_y neg_scale_factor)
			max_y (* temp_min_y neg_scale_factor)))
	(scatter (Lmap) :advance +real_0 :min_x min_x :max_x max_x
...
(defun parse-cff-glyph (buf g_offset len scale_factor)
	(defq idx g_offset end (+ g_offset len)
		cx +real_0 cy +real_0 commands (list) stack (list)
				...
				(cond
					...
					((find b '(26 27))
						...
						(each (lambda (c)
							...
							(defq x1 (+ cx dxa) y1 (- cy dya)
								x2 (+ x1 dxb) y2 (- y1 dyb)
								x3 (+ x2 (if (= b 26) +real_0 dyc)) y3 (+ y2 (if (= b 26) dyc +real_0)))
							(push-curveto x1 y1 x2 y2 x3 y3)
					...
					((find b '(30 31))
						...
						(while (< (+ i 3) sp)
							(defq x1 +real_0 y1 +real_0 x2 +real_0 y2 +real_0 x3 +real_0 y3 +real_0
								extra (if (= (- sp i) 5) (elem-get stack (+ i 4)) +real_0))
							(cond
					...
					((= b 14)
						(setq idx end))
					(:t (clear stack))))))
	(defq min_x +real_0 max_x +real_0 min_y +real_0 max_y +real_0)
	(when (nempty? commands)
		...
		(setq max_x (- max_x min_x) min_x +real_0))
	(scatter (Lmap) :advance +real_0 :min_x min_x :max_x max_x
		:min_y min_y :max_y max_y :commands commands))
			...
			((> g_index 0)
				...
				(unless glyph_db
					(setq glyph_db (scatter (Lmap)
						:min_x +real_0 :max_x +real_0 :min_y +real_0 :max_y +real_0 :commands (list))))
				(. glyph_db :insert :char_code c)
				(. glyph_db :insert :advance (- (. glyph_db :find :max_x) (. glyph_db :find :min_x)))
				(push glyphs glyph_db))
			(:t (push glyphs (scatter (Lmap) :char_code c :offset 0 :advance +real_0
					:min_x +real_0 :max_x +real_0 :min_y +real_0 :max_y +real_0 :commands (list)))))
		(++ c))
...
(defun load-otf-ttf-buf (buf file)
	...
	(defq num_glyphs (get-otf-maxp buf tables)
		scale_factor (/ +real_1 units_per_em)
...
(defun load-ctf-buf (buf)
	(defq ascent (/ (n2r (get-short buf +font_data_ascent)) +real_8192)
		descent (/ (n2r (get-short buf +font_data_descent)) +real_8192)
		xkern (/ (n2r (get-short buf +font_data_xkern)) +real_8192)
	...
	(each (lambda (page_db)
		...
		(each (lambda (glyph_offset)
			(when (or (empty? opt_r) (some (lambda ((s e)) (<= s c e)) opt_r))
				(defq width (/ (n2r (get-short buf (+ glyph_offset +font_path_width))) +real_8192)
				...
				(times klen
					(push kerns (list (get-ushort buf (+ k_offset +font_kern_code))
						(/ (n2r (get-short buf (+ k_offset +font_kern_xkern))) +real_8192)))
				...
				(when (> len 0)
					...
					(while (< g_offset end_g_offset)
						...
						(cond
							((or (= type 0) (= type 1))
								(defq rx (/ (n2r (get-short buf g_offset)) +real_8192)
									ry (/ (n2r (get-short buf (+ g_offset +short_size))) +real_8192))
								(++ g_offset (- +font_line_element_size +font_path_element_size))
								(push coords_x rx)
								(push coords_y ry)
								(push commands (list type rx ry)))
							((= type 2)
								(defq rx1 (/ (n2r (get-short buf g_offset)) +real_8192)
									ry1 (/ (n2r (get-short buf (+ g_offset +short_size))) +real_8192)
									rx2 (/ (n2r (get-short buf (+ g_offset (const (* +short_size 2))))) +real_8192)
									ry2 (/ (n2r (get-short buf (+ g_offset (const (* +short_size 3))))) +real_8192)
									rx (/ (n2r (get-short buf (+ g_offset (const (* +short_size 4))))) +real_8192)
									ry (/ (n2r (get-short buf (+ g_offset (const (* +short_size 5))))) +real_8192))
							...
							((= type 3)
								(defq rx1 (/ (n2r (get-short buf g_offset)) +real_8192)
									ry1 (/ (n2r (get-short buf (+ g_offset +short_size))) +real_8192)
									rx (/ (n2r (get-short buf (+ g_offset (const (* +short_size 2))))) +real_8192)
									ry (/ (n2r (get-short buf (+ g_offset (const (* +short_size 3))))) +real_8192))
...
(defun write-ctf (font_db file)
	(when (and font_db (defq stream (file-stream file +file_open_write)))
		...
		(write-char stream (n2i (* ascent +real_8192)) +short_size)
		(write-char stream (n2i (* descent +real_8192)) +short_size)
		(write-char stream (n2i (* xkern +real_8192)) +short_size)
		...
		(each (lambda (page_db)
			(each (lambda (glyph_db)
					...
					(write-char stream (n2i (* width +real_8192)) +short_size)
					...
					(each (lambda ((type &rest args))
							...
							(cond
								((= type 2)
									...
									(write-char stream (n2i (* x1 +real_8192)) +short_size)
									(write-char stream (n2i (* y1 +real_8192)) +short_size)
									(write-char stream (n2i (* x2 +real_8192)) +short_size)
									(write-char stream (n2i (* y2 +real_8192)) +short_size)
									(write-char stream (n2i (* x3 +real_8192)) +short_size)
									(write-char stream (n2i (* y3 +real_8192)) +short_size))
								((= type 3)
									...
									(write-char stream (n2i (* y2 +real_8192)) +short_size))
								(:t (bind '(x y) args)
									(write-char stream (n2i (* x +real_8192)) +short_size)
									(write-char stream (n2i (* y +real_8192)) +short_size))))
						commands)
					(each (lambda ((c2 adj))
							(write-char stream c2 +short_size)
							(write-char stream (n2i (* adj +real_8192)) +short_size))
						kerns)

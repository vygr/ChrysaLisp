...
(defun parse-cff-glyph (buf g_offset len scale_factor)
	(defq idx g_offset end (+ g_offset len)
		cx 0.0 cy 0.0 commands (list) stack (list))
	(while (< idx end)
		(defq b (get-ubyte buf idx))
		(cond
			((<= 32 b 246)
			...
			(:t
				...
				(cond
					((find b '(4 21 22))
						...
						(push commands (list 0 (* cx scale_factor) (* cy scale_factor)))
						(setq stack (clear stack)))
					((= b 5)
						(each (lambda ((dx dy))
							(++ cx dx) (++ cy dy)
							(push commands (list 1 (* cx scale_factor) (* cy scale_factor))))
							(partition stack 2))
						(setq stack (clear stack)))
					((find b '(6 7))
						(defq horiz (= b 6))
					...
					((= b 8)
						(each (lambda ((dxa dya dxb dyb dxc dyc))
							(defq x1 (+ cx dxa) y1 (+ cy dya)
								x2 (+ x1 dxb) y2 (+ y1 dyb)
								x3 (+ x2 dxc) y3 (+ y2 dyc))
							(push commands (list 2 (* x3 scale_factor) (* y3 scale_factor)
					...
					((find b '(26 27))
						...
						(each (lambda (c)
							...
							(defq x1 (+ cx dxa) y1 (+ cy dya)
								x2 (+ x1 dxb) y2 (+ y1 dyb)
								x3 (+ x2 (if (= b 26) 0.0 dyc)) y3 (+ y2 (if (= b 26) dyc 0.0)))
							(push commands (list 2 (* x3 scale_factor) (* y3 scale_factor)
								(* x1 scale_factor) (* y1 scale_factor)
								(* x2 scale_factor) (* y2 scale_factor)))

		...
		(each (lambda (inst)
			...
			(cond
				((find op '(emit-call-p emit-jmp-p))
					(. deps :insert (resolve-call insts (second inst))))
				((find op '(emit-call-i emit-jmp-i))
					(bind '(& & & &optional c_ m_) inst)
					(setq c c_ m m_))
			...
			(when (and c m)
				(each (lambda (desc_c)
					(when (defq func_name (resolve-method-function *class_db* desc_c m))
...
(defun lookup-trashes-union (c m)
	...
	;method trashes, memoized based on the state of the active db
			...
			(each (lambda (desc_c)
				(when (defq func_name (resolve-method-function *class_db* desc_c m))
					(when (defq f_entry (. db :find func_name))
						(setq found_any :t)
						(defq trashes_set (second f_entry))
...
(defun lookup-trashes-union-from-key (key)
	(defq parts (split (str key) ":"))
	(if (= (length parts) 3)
		(lookup-trashes-union (sym (cat ":" (second parts))) (sym (cat ":" (third parts))))

...
(defun expand-trashes-string (s)
    (defq out (list))
    (each (lambda (part)
            (setq part (trim part))
            (defq parts (split part "-"))
            (cond
                ((and (= (length parts) 2)
                    ...
                    (when (and start_val end_val)
                        (each (lambda (val)
                            (push out (sym (str prefix val))))
				...
				(cond
					...
					((eql op 'emit-call-i)
						(if (bind '(& & & &optional c m) inst)
							(defq call_set (lookup-trashes-union c m))
							(defq call_set +all_extern_trashed_regs))

...
(defq +templates (scatter (Fmap 256)
	...
	"vpif" '(:choice (:flow) (:head :body)) "vpifnot" '(:choice (:flow) (:head :body)) "elseif" '(:choice (:flow) (:head :body)) "elseifnot" '(:choice (:flow) (:head :body))
	"else" '(:flow) "endif" '(:flow) "loop-start" '(:head :body) "loop-while" '(:head :body)
	"loop-whilenot" '(:head :body) "loop-until" '(:flow) "loop-untilnot" '(:flow) "loop-end" '(:flow)
	"switch" '(:head (:clauses 1 1)) "vpcase" '(:choice (:flow) (:head :body)) "vpcasenot" '(:choice (:flow) (:head :body)) "default" '(:flow)
	"endswitch" '(:flow) "goto" '(:flow) "gotoif" '(:flow) "gotoifnot" '(:flow)

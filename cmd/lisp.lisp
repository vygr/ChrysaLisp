...
(defun each (_f &rest _b)
	...
	(cond
		((eq 1 (length _b))
			...
			(while (lt _e _m)
				(setq _a (_f (elem _e _b)) _e (inc _e))))
		((eq 2 (length _b))
			(defq _c (elem 0 _b) _b (elem 1 _b) _m (min (length _b) (length _c)))
			(while (lt _e _m)
				(setq _a (_f (elem _e _c) (elem _e _b)) _e (inc _e))))
		(t
			(defq _m (min-len _b) _i nil)
			(while (lt _e _m)
				(setq _a (list) _i 0)
...
(defun each-rev (_f &rest _b)
	(cond
		((eq 1 (length _b))
			(defq _a nil _b (elem 0 _b) _e (dec (length _b)))
			(while (ge _e 0)
				(setq _a (_f (elem _e _b)) _e (dec _e))))
		((eq 2 (length _b))
			(defq _a nil _c (elem 0 _b) _b (elem 1 _b) _e (dec (min (length _b) (length _c))))
			(while (ge _e 0)
				(setq _a (_f (elem _e _c) (elem _e _b)) _e (dec _e))))
		(t
	...
	(cond
		...
		((eq 2 (length _b))
			...
			(while (lt _e _m)
				...
				(setq _e (inc _e))))
		(t
			(defq _m (min-len _b) _a nil _i nil)
			(while (lt _e _m)
				(setq _a (list) _i 0)
				(while (lt _i (length _b))
					(push _a (elem _e (elem _i _b)))
	...
	(cond
		((eq 1 (length _b))
			...
			(while (and _v (lt _e _m))
				(setq _v (_f (elem _e _b)) _e (inc _e))))
		((eq 2 (length _b))
			(defq _c (elem 0 _b) _b (elem 1 _b) _m (min (length _b) (length _c)))
			(while (and _v (lt _e _m))
				(setq _v (_f (elem _e _c) (elem _e _b)) _e (inc _e))))
		(t
			(defq _m (min-len _b) _i nil)
			(while (and _v (lt _e _m))
				(setq _a (list) _i 0)

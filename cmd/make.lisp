...
(defun information (stream info)
	(when (nempty? info)
		(write-line-lf stream "```code")
		(write-line-lf stream (first info))
		(setq info (rest info))
		(when (nempty? info)
			(write-line-lf stream "")
			(each (# (write-line-lf stream %0)) info))
		(write-line-lf stream (cat "```" +LF))))
...
(defun make-docs ()
	...
	(each (lambda ((name pname methods info))
			...
			(write-line-lf stream (cat "# " name +LF))
			(if pname (write-line-lf stream (cat "## " pname +LF)))
	...
	(defq document "docs/reference/keys.md" current_file ""
		stream (file-stream document +file_open_write))
	(write-line-lf stream (cat "# Key Bindings" +LF))
	(each (lambda ((file name info))
			(unless (eql file current_file)
				(write-line-lf stream (cat "## " file +LF))
	...
	(defq document "docs/reference/functions.md"
		stream (file-stream document +file_open_write))
	(write-line-lf stream (cat "# Functions" +LF))
	(each (lambda ((name info))
			(when (nempty? info)
				(write-line-lf stream (cat "### " name +LF))
				(information stream info)))
	...
	(defq document "docs/reference/macros.md"
		stream (file-stream document +file_open_write))
	(write-line-lf stream (cat "# Macros" +LF))
	(each (lambda ((name info))
			(when (nempty? info)
				(write-line-lf stream (cat "### " name +LF))
				(information stream info)))
	...
	(each (lambda ((job result))
			(write-line-lf stream (cat "## " (slice job 0 -4)))
			(write-line-lf stream "```code")
			(write-line stream result)
			(write-line-lf stream "```"))
	...
	(each (lambda ((cls super &rest mthds))
		...
		(write-line-lf stream (cat "# " cls +LF))
		(unless (eql ":nil" super)
			(write-line-lf stream (cat "## " super +LF)))
		...
		(when (nempty? lisp_mthds)
			(write-line-lf stream (cat "## Lisp Bindings" +LF))
			(each (lambda ((mthd function))
				(when (and (defq i (some (# (if (eql function (first %0)) (!))) ffi_list))
						(defq info (first (second (elem-get ffi_list i)))))
					(write-line-lf stream (cat "### " info +LF)))) lisp_mthds))
		(when (nempty? mthds)
			(write-line-lf stream (cat "## VP methods" +LF))
			(each (lambda ((mthd function))
				(write-line-lf stream (cat "### " mthd " -> " function +LF))
				(when (and (defq i (find function functions))
						(/= 0 (length (defq info (elem-get docs i)))))
					(write-line-lf stream "```code")
					(each (# (write-line-lf stream %0)) info)
					(write-line-lf stream (const (str "```" +LF))))) mthds))

...
(defun make-docs ()
	...
	(defun chop (_)
		(when (defq e (find-rev -1 (char 0x22) _))
			(setq _ (slice 0 e _))
			(slice (inc (find-rev -1 (char 0x22) _)) -1 _)))
			...
			(each-line (lambda (line)
				(when (eql state :y)
					...
					(if (and (> (length s) 0) (starts-with ";" (first s)))
						(push (last docs) (slice (inc (find 0 ";" line)) -1 line))
						(setq state :x)))
				(when (and (eql state :x) (>= (length line) 9))
					...
					(cond
						...
						((and (or (eql _ "call") (eql _ "jump")) (eql (third s) ":repl_error"))
							(when (and (setq line (chop line)) (not (find 0 line syntax)))
								(push syntax line)
	...
	(each (lambda ((cls super &rest methds))
		...
		(when (nempty? lisp_methds)
			...
			(each (lambda ((methd function))
				(when (and (defq i (find 0 function functions))
						(/= 0 (length (defq info (elem-get i docs)))))
					(when (defq i (find 0 "lisp binding" info))
			...
			(case state
				((:function :class :method)
					...
					(if (and (nempty? s) (starts-with ";" (first s)))
						(push info (trim (slice (inc (find 0 ";" line)) -1 line)))
						(setq state :nil)))
				(:keys
					(defq s (split line +char_class_space))
					(if (nempty? s)
...
(defun main ()
	...
	(when (and
		...
		(defq all (find-rev -1 "all" args) boot (find-rev -1 "boot" args)
			platforms (find-rev -1 "platforms" args) docs (find-rev -1 "docs" args)
			it (find-rev -1 "it" args) test (find-rev -1 "test" args))
		(cond
			(test (make-test))

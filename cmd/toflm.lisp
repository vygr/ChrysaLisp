...
"Usage: toflm [options] [path] ...
	...
	options:
		...
		-n --name path: output film filename.
...
(defun main ()
	...
	(when (and
			...
			(defq opt_f 32 opt_n :nil args (options stdio usage)))
		(if (empty? (defq jobs (rest args)))
			;no, so from stdin
			(lines! (# (push jobs %0)) (io-stream 'stdin)))
		(if (not opt_n)
			(print "Error: Output filename (-n) is required.")
			(progn
						...
						(when canvas
							...
							(if first_frame
								(progn
									...
									(CPM-save canvas out_stream opt_f)
									; Overwrite the MPC. header with MLF.
									(stream-seek out_stream 0 0)
									(write-blk out_stream "MLF.")
									(stream-seek out_stream 0 2)

...
%include 'class/class_lisp.inc'
	...
	def_func class/lisp/env_setl_list
		;inputs
		;r0 = lisp object
		;r1 = vars list
		;r2 = vals list
			...
			if {vals->obj_vtable == @class/class_vector}
				...
				if {len1 == len2}
					...
					loop_while {len1 != len2}
						...
						func_call lisp, env_setl, {this, symbol, value}, {value}
						breakif {value->obj_vtable == @class/class_error}
						assign {len1 + 1}, {len1}
					loop_end
				else
					func_call error, create, {"(setl vars vals): not matching lengths", vars}, {value}
				endif
			else
				func_call error, create, {"(setl vars vals): vals not a list", vals}, {value}
			endif
		else
			func_call error, create, {"(setl vars vals): vars not a list", vars}, {value}
		endif

		...
		if {length == 2}
			...
			if {args->obj_vtable == @class/class_vector}
				...
				if {vals->obj_vtable != @class/class_error}
					...
					func_call lisp, env_setl_list, {this, vars, vals}, {vals}
				endif
				func_call ref, deref, {args}
			else
				func_call error, create, {"(setl vars vals) vals is not a list", args}, {vals}
			endif
		else
			func_call error, create, {"(setl vars vals) wrong numbers of args", args}, {vals}
		endif

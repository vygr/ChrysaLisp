...
%include 'class/class_lisp.inc'
	...
	def_function class/lisp/func_mod
		;inputs
		;r0 = lisp object
		;r1 = args
		;outputs
		...
		if {length > 2}
			...
			if {length}
				...
				if {first->obj_vtable == @class/class_boxed_long}
					...
					ifnot {iter}
						...
						static_call lisp, error, {this, "(mod val val ...) vals are not all numbers", args}
					endif
				else
					static_call lisp, error, {this, "(mod val val ...) vals are not all numbers", args}
				endif
			...
			static_call lisp, error, {this, "(mod val val ...) not enough args", args}
		endif

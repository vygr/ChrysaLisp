		...
		if {vars->obj_vtable == @class/class_vector}
			if {vals->obj_vtable == @class/class_vector}
				...
				if {len1 == len2}
					...
					static_call error, create, {"(set vars vals): not matching lengths", vars}, {value}
				endif
			else
				static_call error, create, {"(set vars vals): vals not a list", vals}, {value}
			endif
		else
			static_call error, create, {"(set vars vals): vars not a list", vars}, {value}
		endif

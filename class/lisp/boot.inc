...
(defmacro case (form &rest body)
	...
	(if (defq table (reduce-rev (lambda (table (keys &rest clause_body))
			...
			(unless (some! 0 -1 t (lambda (clause) (find :sym (type-of clause))) (list keys))
...
(defun map (_f &rest _)
	; (map lambda seq ...) -> list
	(defq _l (cap (length (elem 0 _)) (list)))
...
(defun intern-seq (_s &optional _l _c)
	...
	(setd _c eql)
	(reduce (lambda (_l _e)
		(push _l (if (defq _i (some (lambda (_) (if (_c _ _e) _)) _l)) _i _e))) _s (opt _l (list))))
...
(defun str-to-num (_)
	...
	(each (lambda (_) (if (eql _ ".")
			(setq f 1)
			(setq n (+ (* n b) (char-to-num _)) f (* f b)))) _)
...
(defun walk-list (_l _f _fi _fo)
	...
	;if fnc_in returns nil will it will step down into that list.
			...
			((= _ 0)
				...
				(cond
					((_fi _l)
						(_fo _l))
					(t	(push _stack _l 1)
						(if (list? (defq _e (elem 0 _l))) (push _stack _e 0))
						(_f _e))))
			((< _ (length _l))
				;element of list
				(push _stack _l (inc _))
				(if (list? (defq _e (elem _ _l))) (push _stack _e 0))
				(_f _e))
			(t	;exit list
				(_fo _l)))))
...
;debug_mode 2, guarded, with error checking, object stats and guard pages

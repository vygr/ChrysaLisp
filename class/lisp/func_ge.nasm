...
def_func class/lisp/func_ge
	...
	if {length > 1}
		...
		if {first->obj_vtable == @class/class_boxed_long}
			...
			func_call error, create, {"(ge num num ...) not all numbers", first}, {value}
		endif
	else
		func_call error, create, {"(ge num num ...) wrong number of args", args}, {value}
	endif
...
callback:
	...
	if {(*iter)->obj_vtable == @class/class_boxed_long}
		...
		func_call ref, deref, {pdata->pdata_value}
		func_call error, create, {"(ge num num ...) not all numbers", *iter}, {pdata->pdata_value}
	endif

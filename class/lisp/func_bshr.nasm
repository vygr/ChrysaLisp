	...
	if {length == 2}
		...
		if {num->obj_vtable == @class/class_boxed_long}
			...
			if {shift->obj_vtable == @class/class_boxed_long}
				...
				func_call error, create, {"(bit-shr num cnt) not a count", args}, {value}
			endif
		else
			func_call error, create, {"(bit-shr num cnt) not a number", args}, {value}
		endif
	else
